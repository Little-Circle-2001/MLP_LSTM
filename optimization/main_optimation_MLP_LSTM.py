...
idx = 39 # index of the test trajectory (e.g., idx = 18111)
...
if not scenario_test_dataset:
    ...
    states_i, actions_i, obstacles_i, mask_i, timesteps_i, oe, time_discr, time_sec, horizons, ix = test_sample_MLP_LSTM
...
#     model = inference.get_MLP_LSTM_model(network_model_name)
#     inference_func = getattr(inference, 'torch_model_inference_'+transformer_ws+'_no_use_dataset')
#     print('Using MLP_LSTM model \'', network_model_name, '\' with inference function DT_manage.'+inference_func.__name__+'()')
#
...
#
#     MLP_LSTM_trajectory, runtime_MLP_LSTM = inference_func(model, test_loader_MLP_LSTM, hrz, states_rtn, actions_cvx, rtg, ctg, timesteps, attention_mask, oe_hrz, time_hrz, dt_hrz, stm_hrz, cim_hrz, psi_hrz, state_representation, ctg_perc=0.)
#     states_roe_ws_MLP_LSTM = MLP_LSTM_trajectory['roe_' + transformer_ws] # set warm start
#     # states_rtn_ws_MLP_LSTM = MLP_LSTM_trajectory['rtn_' + transformer_ws]
#     actions_rtn_ws_MLP_LSTM = MLP_LSTM_trajectory['dv_' + transformer_ws]
#     states_roe_MLP_LSTM_trg = np.append(states_roe_ws_MLP_LSTM, (states_roe_ws_MLP_LSTM[:,-1]+cim_hrz[:,:,-1].dot(actions_rtn_ws_MLP_LSTM[:,-1])).reshape((6,1)), 1)
#     states_rtn_ws_MLP_LSTM = roe_to_rtn_horizon(states_roe_MLP_LSTM_trg, oe_hrz_trg, n_time_rpod+1)
#     print('MLP_LSTM cost:', la.norm(actions_rtn_ws_MLP_LSTM, axis=0).sum())
#     print('MLP_LSTM runtime:', runtime_MLP_LSTM)
