...
def ocp_cvx_tpbvp(stm, cim, psi, s_0, n_time):
    ...
    if not solve_normalized:
        ...
        # Terminal Condition
        constraints += [s[:,-1] + cim[:,:,-1] @ a[:,-1] == s_f] # 终端约束为什么不用乘转移矩阵？
...
def solve_scp(stm, cim, psi, state_roe_0, states_roe_ref, n_time):
    ...
    diff = trust_region0
    trust_region = trust_region0    # 初始信赖域
...
def check_if_koz(delta_state_rtn_obs, n_time):

    constr_koz = np.empty(shape=(n_time,), dtype=float)
    constr_koz_violation = np.zeros(shape=(n_time,), dtype=float)

    for i in range(n_time):
        constr_koz[i] = np.transpose(delta_state_rtn_obs).dot(EE_koz.dot(delta_state_rtn_obs))
        if (constr_koz[i] < 1) and (i < dock_wyp_sample):
            constr_koz_violation[i] = 1

    return constr_koz, constr_koz_violation

def compute_koz_vector(delta_state_rtn_obs, n_data, n_time, max_num_obstacle):

    constraint_to_go = np.empty(shape=(n_data, n_time, max_num_obstacle, 1), dtype=float)
    for n in range(n_data):
        for i in range(max_num_obstacle):
            constr_koz_n, constr_koz_violation_n = check_koz_constraint(np.transpose(np.squeeze(delta_state_rtn_obs[n, :, i, :])), n_time)
            for t in range(n_time):
                constraint_to_go[n, t, i, 0] = constr_koz_violation_n[t]
