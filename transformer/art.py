...
class AutonomousRendezvousTransformer(DecisionTransformerPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # 嵌入层
        self.embed_timestep = nn.Embedding(config.max_ep_len, config.hidden_size)   # 对时间步进行编码
        self.embed_return = torch.nn.Linear(1, config.hidden_size)        # 对“return-to-go”值进行嵌入
        self.embed_constraint = torch.nn.Linear(1, config.hidden_size)    # 对constraint-to-go进行嵌入
        self.embed_state = torch.nn.Linear(config.state_dim, config.hidden_size)    # 对状态进行嵌入
        self.embed_action = torch.nn.Linear(config.act_dim, config.hidden_size)     # 对采取的动作进行嵌入
        ...
        # 预测头
        self.predict_state = torch.nn.Linear(config.hidden_size, config.state_dim) # 预测下一时刻的状态
