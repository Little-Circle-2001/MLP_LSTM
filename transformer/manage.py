...
def import_dataset_for_DT_eval_vXX(model_name):
    ...
    torch_states = torch.concatenate((states_scp, states_cvx), axis=0)
    torch_actions = torch.concatenate((actions_scp, actions_cvx), axis=0)
    torch_rtgs = torch.concatenate((rtgs_scp, rtgs_cvx), axis=0)
    torch_ctgs = torch.concatenate((ctgs_scp, ctgs_cvx), axis=0)

    num_samples = len(torch_states)  # 根据数据的实际维度来获得样本数
    # 生成随机排列的索引
    perm = np.random.permutation(num_samples)

    torch_states = torch_states[perm]
    torch_actions = torch_actions[perm]
    torch_rtgs = torch_rtgs[perm]
    torch_ctgs = torch_ctgs[perm]
    data_param = {
        'horizons' : np.concatenate((data_param['horizons'], data_param['horizons']), axis=0)[perm],
        'time_discr' : np.concatenate((data_param['dtime'], data_param['dtime']), axis=0)[perm],
        'time_sec' : np.concatenate((data_param['time'], data_param['time']), axis=0)[perm],
...
def torch_model_inference_dyn_no_use_dataset(model, test_loader, horizons, states_i, actions_i, rtgs_i, ctgs_i, timesteps_i, attention_mask_i, oe, time_sec, dt, stm, cim, psi, state_representation, ctg_perc=1.):
    # Get dimensions and statistics from the dataset
    n_state = 6
    n_time = 100
    n_action = 3
    ...
    ctgs_i = torch.from_numpy(ctgs_i).view(1, n_time, 1).to(device)  # probably not needed??
    states_i = torch.from_numpy(states_i).to(device)
    timesteps_i = torch.tensor(timesteps_i).long().to(device)
    attention_mask_i = torch.from_numpy(attention_mask_i).long().to(device)
    oe = torch.from_numpy(oe).view(1,n_state,n_time,1).float().to(device)
    time_sec = torch.from_numpy(time_sec).view(1, 1, n_time).float().to(device)
    ...
    runtime0_DT = time.time()
    # Dynamics-in-the-loop initialization
    states_dyn[:, 0, :] = (states_i[:, 0, :] - data_stats['states_mean'][0]) / data_stats['states_std'][0]
    ...
    for t in np.arange(n_time):
        ...
        if t != n_time - 1:
            ...
            rtn_dyn[:, t + 1] = psi[:, :, t + 1] @ roe_dyn[:, t + 1]
            if state_representation == 'roe':
                states_dyn_norm = (roe_dyn[:, t + 1] - data_stats['states_mean'][t + 1]) / (
                            data_stats['states_std'][t + 1] + 1e-6)
            elif state_representation == 'rtn':
                states_dyn_norm = (rtn_dyn[:, t + 1] - data_stats['states_mean'][t + 1]) / (
                            data_stats['states_std'][t + 1] + 1e-6)
            states_dyn[:, t + 1, :] = states_dyn_norm
