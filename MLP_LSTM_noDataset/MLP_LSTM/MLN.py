...
class MLP_LSTM_Network(nn.Module):
    def __init__(self, config):
        ...
        self.LSTM = nn.LSTM(
            input_size=config.MLP_output_size + config.action_dim + 1,
    ...
    def forward(self, observe, delta_observe_target, obstacle, mask, action, time):
        ...
        #     x = self.MLP_out(x)                 # [batch_size, MLP_output_dim]
        #     MLP_out.append(x.unsqueeze(1))      # [batch_size, 1, MLP_action_dim]
        # MLP_out = torch.cat(MLP_out, dim=1)     # [batch_size, seq_len, MLP_output_dim]
        ...
        LSTM_input = torch.cat((MLP_out, action, time), dim=-1)     # [batch_size, seq_len, MLP_action_dim + action_dim + 1]
        LSTM_out, (hn, cn)= self.LSTM(LSTM_input, (h0, c0))                 # [batch_size, sen_len, hidden_size]
        output = self.LSTM_out(LSTM_out)                                    # [batch_size, seq_len, action_dim]
...
# action = torch.randn(32, 100, config.action_dim)
#
