...
def model_inference_dyn(model, observe0, target, obstacle, action0, stm, cim, psi, state_representation, seq_len):
    ...
    if state_representation == 'roe':
        state_dyn[:, 0, :] = (roe_dyn[:, 0] - states_mean[0]) / states_std[0]
        state_trg_dyn[:, 0, :] = (roe_trg_dyn[:, 0] - delta_states_target_mean[0]) / delta_states_target_std[0]
    elif state_representation == 'rtn':
        state_dyn[:, 0, :] = (rtn_dyn[:, 0] - states_mean[0]) / states_std[0]
        state_trg_dyn[:, 0, :] = (rtn_trg_dyn[:, 0] - delta_states_target_mean[0]) / delta_states_target_std[0]
    state_obstacle_dyn[:, 0, :, :] = torch.cat(((obstacle_dyn[0, :, :3] - obstacle_mean[0]) / obstacle_std[0], obstacle_dyn[0, :, 3].unsqueeze(-1)), dim=-1)
    ...
    for t in range(seq_len):
        with torch.no_grad():
            ...
            output = outputs[:, -1, :]                                      # [batch_size=1, action_dim]
        action_dyn[:, t, :] = output                                    # [batch_size=1, 1, action_dim]
        ...
        if t != seq_len - 1:
            ...
            state_obstacle_dyn[:, t + 1, :, :] = torch.cat((obstacle_dyn[t + 1, :, :3] - obstacle_mean[t + 1] / obstacle_std[t + 1], obstacle_dyn[t + 1, :, 3].unsqueeze(-1)), dim=-1)
            ...
            time_inputs[:, max(-config.seq_len, -(t + 2)):, :] = timestep_dyn[:, max(0, t - config.seq_len + 2):(t + 2), :]
