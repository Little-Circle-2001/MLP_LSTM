...
idx = 128 # index of the test trajectory (e.g., idx = 18111)
...
if not scenario_test_dataset:
    ...
    test_dataset, test_loader = inference.get_test_data()
    ...
    states_i, actions_i, obstacles_i, mask_i, timesteps_i, oe, time_discr, time_sec, horizons, ix = test_sample
...
if warmstart == 'transformer' or warmstart == 'both':
    ...
    print('Using ART model \'', network_model_name, '\' with inference function inference.'+inference_func.__name__+'()')
    action0 = np.array([0, 0, 0])
    DT_trajectory, runtime_DT = inference_func(model, state_roe_0, state_rtn_target, obstacle, action0, stm_hrz, cim_hrz, psi_hrz, state_representation, n_time_rpod)
    states_roe_ws_DT = DT_trajectory['roe_' + transformer_ws] # set warm start
    # states_rtn_ws_DT = DT_trajectory['rtn_' + transformer_ws]
    actions_rtn_ws_DT = DT_trajectory['control_' + transformer_ws]
...
#     print('Using ART model \'', network_model_name, '\' with inference function DT_manage.'+inference_func.__name__+'()')
#
