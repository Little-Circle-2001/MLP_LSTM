...
def model_inference_dyn(model, observe0, target, obstacle, action0, stm, cim, psi, state_representation, seq_len):
    ...
    time_seq = torch.tensor(range(seq_len)).view(seq_len, 1)
    timestep_dyn = torch.empty(1, seq_len, 1)
    ...
    observe_inputs = state_dyn[:, 0, :].unsqueeze(0)
    observe_trg_inputs = state_trg_dyn[:, 0, :].unsqueeze(0)
    observe_obstacle_inputs = state_obstacle_dyn[:, 0, :, :].unsqueeze(0)
    mask_inputs = obstacle_mask[:, 0, :].unsqueeze(0)
    action_inputs = action0.unsqueeze(0).unsqueeze(0)
    time_inputs = timestep_dyn[:, 0, :].unsqueeze(0)


    for t in range(seq_len):
        if t <= config.seq_len - 2:
            with torch.no_grad():
                outputs = model(
            ...
            output = outputs[:, t, :3]                                  # [batch_size=1, action_dim]
            action_dyn[:, t, :] = output                                # [batch_size=1, action_dim]

            action_inputs = torch.cat((action_inputs, action_dyn[:, t, :].unsqueeze(0)), dim=1)
            ...
            observe_inputs = state_dyn[:, :(t + 2), :]
            observe_trg_inputs = state_trg_dyn[:, :(t + 2), :]
            observe_obstacle_inputs = state_obstacle_dyn[:, :(t + 2), :, :]
            mask_inputs = obstacle_mask[:, :(t + 2), :]
            time_inputs = timestep_dyn[:, :(t + 2), :]
            ...
            output = outputs[:, -1, :3]                                  # [batch_size=1, action_dim]
            action_dyn[:, t, :] = output                                # [batch_size=1, action_dim]

            action_inputs = torch.cat((action_inputs[:, 1:, :], action_dyn[:, t, :].unsqueeze(0)), dim=1)
            ...
            if t != seq_len - 1:
                ...
                observe_inputs = state_dyn[:, (t - config.seq_len + 2):(t + 2), :]                              # [batch_size=1, seq_len, state_dim]
                observe_trg_inputs = state_trg_dyn[:, (t - config.seq_len + 2):(t + 2), :]                      # [batch_size=1, seq_len, state_dim]
                observe_obstacle_inputs = state_obstacle_dyn[:, (t - config.seq_len + 2):(t + 2), :, :]     # [batch_size=1, seq_len, max_num_obstacle, obstacle_dim]
                mask_inputs = obstacle_mask[:, (t - config.seq_len + 2):(t + 2), :]                            # [batch_size=1, seq_len, max_num_obstacle]
                time_inputs = timestep_dyn[:, (t - config.seq_len + 2):(t + 2), :]
