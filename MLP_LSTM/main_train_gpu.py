...
class MLP_LSTM_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        observes = torch.stack([self.data['observes'][i, :, :] for i in ix]).view(self.seq_len, observe_dim).float()
        delta_observes_target = torch.stack([self.data['observes_target'][i, :, :] for i in ix]).view(self.seq_len, observe_dim).float()
        obstacles = torch.stack([self.data['obstacles'][i, :, :, :] for i in ix]).view(self.seq_len, max_num_obs, obstacle_dim).float()
        actions = torch.stack([self.data['actions'][i, :, :] for i in ix]).view(self.seq_len, action_dim).float()
        mask = torch.stack([self.data['masks'][i, :, :] for i in ix]).view(self.seq_len, max_num_obs)
...
print(f"MLP_LSTM size: {model_size/1000**2:.1f}M parameters")
model.to(device);
...
def evaluate():
    ...
    for step, batch in enumerate(eval_loader):
        ...
        observes_inputs = torch.empty(size=(batch_size, seq_len, observe_dim), device=device).float()
        delta_observes_target_inputs = torch.empty(size=(batch_size, seq_len, observe_dim), device=device).float()
        obstacles_inputs = torch.empty(size=(batch_size, seq_len, config.max_obstacle, obstacle_dim), device=device).float()
        mask_inputs = torch.empty(size=(batch_size, seq_len, max_num_obs), device=device).float()
        actions_inputs = torch.empty(size=(batch_size, seq_len, action_dim), device=device).float()
        timestep_inputs = torch.zeros(size=(batch_size, seq_len, 1), device=device).float()

        observes_inputs[:, :, :] = observes_i
        delta_observes_target_inputs[:, :, :] = delta_observes_target_i
        obstacles_inputs[:, :, :, :] = obstacles_i
        mask_inputs[:, :, :] = mask_i
        actions_inputs[:, :, :] = torch.cat((torch.zeros(size=(batch_size, 1, action_dim), device=device).float(), actions_i[:, :-1, :]), dim=1)
        timestep_inputs[:, :, :] = timesteps_i

        outputs = torch.empty(size=(batch_size, seq_len, config.action_dim), device=device)
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        accelerator.print('第{}轮训练开始，共{}轮'.format(epoch + 1, epochs))
        ...
        if (epoch + 1) % eval_epoch*10 == 0:
           accelerator.save_state(root_folder + '/MLP_LSTM/saved_files/checkpoints/checkpoint_MLP_LSTM_v8')
        accelerator.print('第{}轮训练结束，共{}轮'.format(epoch + 1, epochs))
