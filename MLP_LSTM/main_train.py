...
def evaluate():
    ...
    for step, batch in enumerate(eval_loader):
        ...
        time = torch.tensor(range(1 - config.seq_len, seq_len)).view(seq_len + config.seq_len - 1, 1)
        timesteps_i = torch.empty(batch_size, seq_len + config.seq_len - 1, 1)
        timesteps_i[:, :, :] = (seq_len - 1 - time) / (seq_len - 1)
        ...
        actions_inputs[:, :, :] = 0
        timestep_inputs[:, :, :] = timesteps_i[:, :config.seq_len, :]

        outputs = torch.zeros((batch_size, seq_len, config.action_dim))
        ...
        for timestep in range(seq_len):
            ...
            timestep_inputs[:, :, :] = timesteps_i[:, timestep:timestep+config.seq_len, :]
            ...
            output = output[:, min(timestep, config.seq_len-1), :]
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        for step, batch in enumerate(train_loader):
            ...
            masking_loss = torch.zeros(batch_size, config.seq_len, 1)
            ...
            for timestep in range(seq_len):
                ...
                control = output[:, :, :3]
                state = output[:, :, 3:]
                masking_loss[:, max(-config.seq_len, -(timestep + 1)):, :] = 1

                loss_control = torch.mean(((control[:, :min(timestep + 1, config.seq_len), :] - actions_inputs[:, max(-config.seq_len, -(timestep + 1)):, :])) ** 2)
                if timestep == 0:
                    loss_state = 0
                else:
                    loss_state = torch.mean(((state[:, :min(timestep + 1, config.seq_len) - 1, :] - observes_inputs[:, max(-config.seq_len, -(timestep + 1)) + 1:, :])) ** 2)
        ...
        if (epoch + 1) % eval_epoch*10 == 0:
           torch.save(model.state_dict(), f=root_folder+'\MLP_LSTM\saved_files\checkpoints\checkpoint_MLP_LSTM_v6.pth')
