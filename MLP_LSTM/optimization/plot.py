...
from optimization.main_optimation_MLP_LSTM import *
...
if warmstart == 'MLP_LSTM' or warmstart == 'both':
    p3 = ax1.plot3D(states_rtn_ws_MLP_LSTM[1,:t_ws_show], states_rtn_ws_MLP_LSTM[2,:t_ws_show], states_rtn_ws_MLP_LSTM[0,:t_ws_show], 'b--', linewidth=2.5, label='MLP_LSTM') # 'warm-start MLP_LSTM-' + transformer_ws
    if not exclude_scp_DT:
        p4 = ax1.plot3D(states_rtn_scp_MLP_LSTM[1,:], states_rtn_scp_MLP_LSTM[2,:], states_rtn_scp_MLP_LSTM[0,:], 'b-', linewidth=0) #scp (MLP_LSTM-' + transformer_ws + ')_(' + str(iter_scp_MLP_LSTM) + ')
if warmstart == 'transformer' or warmstart == 'both':
    p5 = ax1.plot3D(states_rtn_ws_DT[1,:t_ws_show], states_rtn_ws_DT[2,:t_ws_show], states_rtn_ws_DT[0,:t_ws_show], 'g--', linewidth=2.5, label='ART') # 'warm-start ART-' + transformer_ws
    if not exclude_scp_DT:
        p6 = ax1.plot3D(states_rtn_scp_DT[1,:], states_rtn_scp_DT[2,:], states_rtn_scp_DT[0,:], 'g-', linewidth=0) #scp (ART-' + transformer_ws + ')_(' + str(iter_scp_DT) + ')pwyp = ax1.scatter(dock_wyp[1], dock_wyp[2], dock_wyp[0], color = 'r', marker = '*', linewidth=2.5)
...
p7 = ax1.scatter(states_rtn_scp_cvx[1,-1], states_rtn_scp_cvx[2,-1], states_rtn_scp_cvx[0,-1], color = 'g', marker = '*', linewidth=1.5)
...
# if warmstart == 'transformer' or warmstart == 'both':
#     p3 = ax2.plot3D(states_rtn_ws_MLP_LSTM[1,:t_ws_show], states_rtn_ws_MLP_LSTM[2,:t_ws_show], states_rtn_ws_MLP_LSTM[0,:t_ws_show], 'b-', linewidth=0) # 'warm-start MLP_LSTM-' + transformer_ws
#     if not exclude_scp_DT:
#         p4 = ax2.plot3D(states_rtn_scp_MLP_LSTM[1,:], states_rtn_scp_MLP_LSTM[2,:], states_rtn_scp_MLP_LSTM[0,:], 'b-', linewidth=2.5, label='SCP-MLP_LSTM') #scp (MLP_LSTM-' + transformer_ws + ')_(' + str(iter_scp_MLP_LSTM) + ')
if warmstart == 'transformer' or warmstart == 'both':
    p5 = ax2.plot3D(states_rtn_ws_DT[1,:t_ws_show], states_rtn_ws_DT[2,:t_ws_show], states_rtn_ws_DT[0,:t_ws_show], 'g-', linewidth=0) # 'warm-start ART-' + transformer_ws
    if not exclude_scp_DT:
        p6 = ax2.plot3D(states_rtn_scp_DT[1,:], states_rtn_scp_DT[2,:], states_rtn_scp_DT[0,:], 'g-', linewidth=2.5, label='SCP-ART') #scp (ART-' + transformer_ws + ')_(' + str(iter_scp_DT) + ')
...
p5 = ax2.plot3D(relativeorbit_0[1,:], relativeorbit_0[2,:], relativeorbit_0[0,:], '-.', color='gray', linewidth=1.5, label='initial rel. orbit')
p6 = ax2.scatter(states_rtn_scp_cvx[1,0], states_rtn_scp_cvx[2,0], states_rtn_scp_cvx[0,0], color = 'b', marker = 'o', linewidth=2.5, label='$t_0$')
p7 = ax2.scatter(states_rtn_scp_cvx[1,-1], states_rtn_scp_cvx[2,-1], states_rtn_scp_cvx[0,-1], color = 'g', marker = '*', linewidth=2.5, label='docking port')
ax2.set_xlabel('\n$\delta r_t$ [m]', fontsize=15, linespacing=1.5)
...
if warmstart == 'MLP_LSTM' or warmstart == 'both':
    plt.plot(time_hrz_trg[:t_ws_show]/period_ref, constr_MLP_LSTM[:t_ws_show], 'b--', linewidth=1.5, label='MLP_LSTM')
    if not exclude_scp_DT:
        plt.plot(time_hrz_trg/period_ref, constr_scp_MLP_LSTM, 'b-', linewidth=1.8, label='SCP-MLP_LSTM')

if warmstart == 'transformer' or warmstart == 'both':
    plt.plot(time_hrz_trg[:t_ws_show]/period_ref, constr_DT[:t_ws_show], 'g--', linewidth=1.5, label='ART')
    if not exclude_scp_DT:
        plt.plot(time_hrz_trg/period_ref, constr_scp_DT, 'g-', linewidth=1.8, label='SCP-ART')

plt.plot(time_hrz_trg/period_ref, np.ones(n_time_rpod+1), 'r-', linewidth=1.8, label='koz')
plt.xlabel('time [orbits]', fontsize=15)
plt.ylabel('keep-out-zone value [-]', fontsize=15)
# plt.grid(True)
plt.xlim([-0.1,3])
plt.ylim([-0.1,9])
plt.legend(loc='best')
...
for j in range(3):
    ...
    plt.plot(time_hrz[:t_ws_show]/period_ref, (actions_rtn_ws_MLP_LSTM[j,:t_ws_show]-actions_rtn_ws_DT[j,:t_ws_show])*1000., linewidth=2.5)
    if j == 0:
...
for j in range(3):
    ...
    plt.plot(time_hrz[:t_ws_show]/period_ref, (states_rtn_ws_MLP_LSTM[j,:t_ws_show]-states_rtn_ws_DT[j,:t_ws_show]), linewidth=2.5)
    if j == 0:
        plt.xlabel('time [orbits]', fontsize=20)
        plt.ylabel('$ \Delta r_r$ [m]', fontsize=20)
        # plt.grid(True)
        plt.xlim([-0.1,3])
        # plt.ylim([-15,25])
        # plt.legend(loc='best', fontsize=15)
        ...
        plt.xlabel('time [orbits]', fontsize=20)
        plt.ylabel('$ \Delta r_t$ [m]', fontsize=20)
        # plt.grid(True)
        plt.xlim([-0.1,3])
        # plt.ylim([-25,25])
        plt.tick_params(axis='y', labelsize=20)
        ...
        plt.xlabel('time [orbits]', fontsize=20)
        plt.ylabel('$ \Delta r_n$ [m]', fontsize=20)
        # plt.grid(True)
        plt.xlim([-0.1,3])
        # plt.ylim([-60,60])
        plt.tick_params(axis='y', labelsize=20)
...
if warmstart == 'MLP_LSTM' or warmstart == 'both':
    p3 = ax1.plot3D(states_rtn_ws_MLP_LSTM[1,:t_ws_show], states_rtn_ws_MLP_LSTM[2,:t_ws_show], states_rtn_ws_MLP_LSTM[0,:t_ws_show], 'b--', linewidth=2.5, label='MLP_LSTM') # 'warm-start MLP_LSTM-' + transformer_ws
    if not exclude_scp_DT:
        p4 = ax1.plot3D(states_rtn_scp_MLP_LSTM[1,:], states_rtn_scp_MLP_LSTM[2,:], states_rtn_scp_MLP_LSTM[0,:], 'b-', linewidth=3, label='SCP-MLP_LSTM') #scp (MLP_LSTM-' + transformer_ws + ')_(' + str(iter_scp_MLP_LSTM) + ')
if warmstart == 'transformer' or warmstart == 'both':
    p5 = ax1.plot3D(states_rtn_ws_DT[1,:t_ws_show], states_rtn_ws_DT[2,:t_ws_show], states_rtn_ws_DT[0,:t_ws_show], 'g--', linewidth=2.5, label='ART') # 'warm-start ART-' + transformer_ws
    if not exclude_scp_DT:
        p6 = ax1.plot3D(states_rtn_scp_DT[1,:], states_rtn_scp_DT[2,:], states_rtn_scp_DT[0,:], 'g-', linewidth=3, label='SCP-ART') #scp (ART-' + transformer_ws + ')_(' + str(iter_scp_DT) + ')
...
#if not exclude_scp_cvx:
p7 = ax1.scatter(states_rtn_scp_cvx[1,0], states_rtn_scp_cvx[2,0], states_rtn_scp_cvx[0,0], color = 'b', marker = 'o', linewidth=1.5, label='$t_0$')
p8 = ax1.scatter(states_rtn_scp_cvx[1,-1], states_rtn_scp_cvx[2,-1], states_rtn_scp_cvx[0,-1], color = 'g', marker = '*', linewidth=1.5, label='docking port')
ax1.set_xlabel('\n$\delta r_t$ [m]', fontsize=15, linespacing=1.5)
ax1.set_ylabel('$\delta r_n$ [m]', fontsize=15)
ax1.set_zlabel('$\delta r_r$ [m]', fontsize=15)
ax1.tick_params(axis='y', labelcolor='k', labelsize=15)
...
pcone._facecolors2d = pcone._facecolor3d
pcone._edgecolors2d = pcone._edgecolor3d
p7 = ax1.plot3D(relativeorbit_0[1,:], relativeorbit_0[2,:], relativeorbit_0[0,:], '-.', color='gray', linewidth=1.5)
#if not exclude_scp_cvx:
p8 = ax1.scatter(states_rtn_scp_cvx[1,0], states_rtn_scp_cvx[2,0], states_rtn_scp_cvx[0,0], color = 'b', marker = 'o', linewidth=1.5)
p9 = ax1.scatter(states_rtn_scp_cvx[1,-1], states_rtn_scp_cvx[2,-1], states_rtn_scp_cvx[0,-1], color = 'g', marker = '*', linewidth=1.5)
ax1.set_xlabel('\n$\delta r_t$ [m]', fontsize=15, linespacing=1.5)
ax1.set_ylabel('$\delta r_n$ [m]', fontsize=15)
ax1.set_zlabel('$\delta r_r$ [m]', fontsize=15)
ax1.tick_params(axis='y', labelcolor='k', labelsize=15)
...
pcone._facecolors2d = pcone._facecolor3d
pcone._edgecolors2d = pcone._edgecolor3d
p7 = ax2.plot3D(relativeorbit_0[1,:], relativeorbit_0[2,:], relativeorbit_0[0,:], '-.', color='gray', linewidth=1.5, label='initial rel. orbit')
p8 = ax2.scatter(states_rtn_scp_cvx[1,0], states_rtn_scp_cvx[2,0], states_rtn_scp_cvx[0,0], color = 'b', marker = 'o', linewidth=2.5, label='$t_0$')
p9 = ax2.scatter(states_rtn_scp_cvx[1,-1], states_rtn_scp_cvx[2,-1], states_rtn_scp_cvx[0,-1], color = 'g', marker = '*', linewidth=2.5, label='docking port')
ax2.set_xlabel('\n$\delta r_t$ [m]', fontsize=15, linespacing=1.5)
ax2.set_ylabel('$\delta r_n$ [m]', fontsize=15)
ax2.set_zlabel('$\delta r_r$ [m]', fontsize=15)
ax2.tick_params(axis='y', labelcolor='k', labelsize=15)
...
# plt.subplots_adjust(wspace=-0.01)
handles, labels = [(a + b) for a, b in zip(ax1.get_legend_handles_labels(), ax2.get_legend_handles_labels())]
first_legend = plt.legend(handles, labels, loc='lower center', bbox_to_anchor=(-0., 0.8),
          ncol=5, fancybox=True, shadow=True, fontsize=15)
ax2.add_artist(first_legend)
...
if warmstart == 'MLP_LSTM' or warmstart == 'both':
    ax.plot(time_hrz_trg[:t_ws_show]/period_ref, constr_MLP_LSTM[:t_ws_show], 'b--', linewidth=1.8, label='MLP_LSTM')
    if not exclude_scp_DT:
        ax.plot(time_hrz_trg/period_ref, constr_scp_MLP_LSTM, 'b-', linewidth=2, label='SCP-MLP_LSTM')

if warmstart == 'transformer' or warmstart == 'both':
    ax.plot(time_hrz_trg[:t_ws_show] / period_ref, constr_DT[:t_ws_show], 'g--', linewidth=1.8, label='ART')
    if not exclude_scp_DT:
        ax.plot(time_hrz_trg / period_ref, constr_scp_DT, 'g-', linewidth=2, label='SCP-ART')
...
ax.set_ylabel('Keep-out-zone value [-]', fontsize=16)
...
# if warmstart == 'transformer' or warmstart == 'both':
#     p3 = plt.plot(states_roe_MLP_LSTM_trg[1,:t_ws_show], states_roe_MLP_LSTM_trg[0,:t_ws_show], 'b--', linewidth=1.2, label='MLP_LSTM')
#     if not exclude_scp_DT:
#         p4 = plt.plot(states_roe_scp_MLP_LSTM_trg[1, :], states_roe_scp_MLP_LSTM_trg[0,:], 'b-', linewidth=1.5, label='SCP-MLP_LSTM')
...
# if warmstart == 'transformer' or warmstart == 'both':
#     p3 = plt.plot(states_roe_MLP_LSTM_trg[2,:t_ws_show], states_roe_MLP_LSTM_trg[3,:t_ws_show], 'b--', linewidth=1.2, label='MLP_LSTM')
#     if not exclude_scp_DT:
#         p4 = plt.plot(states_roe_scp_MLP_LSTM_trg[2, :], states_roe_scp_MLP_LSTM_trg[3,:], 'b-', linewidth=1.5, label='SCP-MLP_LSTM')
...
# if warmstart == 'transformer' or warmstart == 'both':
#     p3 = plt.plot(states_roe_MLP_LSTM_trg[4,:t_ws_show], states_roe_MLP_LSTM_trg[5,:t_ws_show], 'b--', linewidth=1.2, label='MLP_LSTM')
#     if not exclude_scp_DT:
#         p4 = plt.plot(states_roe_scp_MLP_LSTM_trg[4, :], states_roe_scp_MLP_LSTM_trg[5,:], 'b-', linewidth=1.5, label='SCP-MLP_LSTM')
...
for j in range(6):
    ...
    if warmstart == 'MLP_LSTM' or warmstart == 'both':
        plt.plot(time_hrz_trg[:t_ws_show]/period_ref, states_roe_MLP_LSTM_trg[j,:t_ws_show], 'b--', linewidth=1.5, label='MLP_LSTM')
        if not exclude_scp_DT:
            plt.plot(time_hrz_trg/period_ref, states_roe_scp_MLP_LSTM_trg[j,:], 'b-', linewidth=1.8, label='SCP-MLP_LSTM')
    if warmstart == 'transformer' or warmstart == 'both':
        plt.plot(time_hrz_trg[:t_ws_show]/period_ref, states_roe_DT_trg[j,:t_ws_show], 'g--', linewidth=1.5, label='ART')
        if not exclude_scp_DT:
            plt.plot(time_hrz_trg/period_ref, states_roe_scp_DT_trg[j,:], 'g-', linewidth=1.8, label='SCP-ART')
    if j == 0:
...
for j in range(3):
    ...
    plt.stem(time_hrz[:t_ws_show]/period_ref, actions_rtn_ws_MLP_LSTM[j,:t_ws_show]*1000., 'b--', markerfmt='D', label='MLP_LSTM')
    plt.stem(time_hrz/period_ref, actions_scp_MLP_LSTM[j,:]*1000., 'b-', label='SCP-MLP_LSTM')
    plt.stem(time_hrz[:t_ws_show] / period_ref, actions_rtn_ws_DT[j, :t_ws_show] * 1000., 'g--', markerfmt='D', label='ART')
    plt.stem(time_hrz / period_ref, actions_scp_DT[j, :] * 1000., 'g-', label='SCP-ART')
    if j == 0:
        plt.xlabel('time [orbits]', fontsize=20)
        plt.ylabel('$ \Delta v_r$ [mm/s]', fontsize=20)
        # plt.grid(True)
...
#     plt.figure()
#     max_it = max(iter_scp_cvx, iter_scp_MLP_LSTM)
#     for i in range(max_it):
#         if i >= iter_scp_cvx:
#             J_vect_scp_cvx[i] = J_vect_scp_cvx[iter_scp_cvx-1]
#         elif i >= iter_scp_MLP_LSTM:
#             J_vect_scp_MLP_LSTM[i] = J_vect_scp_MLP_LSTM[iter_scp_MLP_LSTM-1]
#     if warmstart == 'cvx' or warmstart == 'both':
#         plt.plot(J_vect_scp_cvx[:max_it]*1000., 'b--', marker='o', linewidth=1.5, label='SCP-CVX')

#     if warmstart == 'transformer' or warmstart == 'both':
#         plt.plot(J_vect_scp_MLP_LSTM[:max_it]*1000., 'g--', marker='o', linewidth=1.5, label='SCP-MLP_LSTM')
...
# io.savemat('plot_data.mat', dict(states_rtn_ws_cvx=states_rtn_ws_cvx, states_rtn_scp_cvx=states_rtn_scp_cvx, states_rtn_ws_MLP_LSTM=states_rtn_ws_MLP_LSTM, states_rtn_scp_MLP_LSTM=states_rtn_scp_MLP_LSTM, dock_wyp=dock_wyp, x_ell=x_ell, y_ell=y_ell, z_ell=z_ell, t_cone=t_cone, n_cone=n_cone, r_cone=r_cone))
