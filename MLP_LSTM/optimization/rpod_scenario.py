...
state_rtn_target = np.array([dock_port.item(0), dock_port.item(1), dock_port.item(2), 0, 0, 0]).reshape((6,))   # RTN坐标系目标状态为 [0 74 0 0 0 0]
...
# Obstacle position
obstacle1 = np.array([[0, 0, 0]])
obstacle2 = np.array([[1, 2, 3]])
obstacle3 = np.array([[4, 5, 6]])
obstacle4 = np.array([[7, 8, 9]])
obstacle5 = np.array([[10, 11, 12]])

obstacle = np.concatenate((obstacle1, obstacle2, obstacle3, obstacle4, obstacle5), axis = 0)
max_num_obstacle = obstacle.shape[0]